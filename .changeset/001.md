...
- `createAttachmentsPlugin({ accept, onUpload, onError?, maxSize? })` — paste
  ...
  remote `src` URL.
- `createSlashCommandsPlugin({ commands, getMarkdown, setMarkdown, onReadyChange? })`
  — reusable slash-command autocomplete for chat-style composers. Supports
  command filtering, disabled commands, required first-argument choices,
  async choice loading, completion readiness, keyboard navigation, and renders
  through the shared `.inkwell-plugin-picker-*` menu classes.
