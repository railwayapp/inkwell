...
in the bottom-right of the editor wrapper. Below the limit the readout is
muted; above it the readout turns red, the wrapper picks up the
`inkwell-editor-over-limit` class, and the editor surface gets a red
border. The `overLimit` field on `InkwellEditorState` reflects the same
condition so consumers can disable submit buttons, etc.
