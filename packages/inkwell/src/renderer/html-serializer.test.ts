...
import { htmlToMarkdown } from "./html-serializer";
...
describe("htmlToMarkdown", () => {
  ...
  it("converts strike tags to markdown strikethrough", () => {
    const md = htmlToMarkdown("<p><strike>deleted</strike></p>");
    expect(md).toContain("~~deleted~~");
  });
  ...
  it("converts inline code", () => {
    const md = htmlToMarkdown("<p><code>foo()</code></p>");
  ...
  it("converts nested inline marks", () => {
    const md = htmlToMarkdown("<p><strong><em>bold italic</em></strong></p>");
    expect(md).toContain("**");
  ...
  it("handles plain text without HTML tags", () => {
    const md = htmlToMarkdown("just text");
  ...
  it("does not escape markdown syntax in text nodes", () => {
    ...
    expect(htmlToMarkdown("<p>**bold**</p>")).toBe("**bold**");
    expect(htmlToMarkdown("<p>_italic_</p>")).toBe("_italic_");
  ...
  it("preserves strikethrough despite table removal", () => {
    const md = htmlToMarkdown("<p><del>struck</del></p>");
